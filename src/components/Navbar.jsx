    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all ${scrolled ? 'backdrop-blur-xl bg-slate-900/60 border-b border-white/10' : 'bg-transparent'}`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <a href="#contact" className="inline-flex items-center rounded-full bg-gradient-to-r from-cyan-500 to-indigo-500 text-white px-4 py-2 text-sm font-semibold shadow-lg shadow-cyan-500/20 hover:shadow-indigo-500/20 transition-all">
            Let’s Talk
          </a>
        </nav>

        <button onClick={() => setOpen(!open)} className="md:hidden text-white p-2 rounded hover:bg-white/10 transition">
      ...
      {open && (
        <div className="md:hidden border-t border-white/10 bg-slate-900/80 backdrop-blur-xl">
          <div className="px-4 py-3 space-y-2">
            ...
            <a href="#contact" onClick={() => setOpen(false)} className="block px-3 py-2 rounded-lg bg-gradient-to-r from-cyan-500 to-indigo-500 text-white text-center font-semibold">
              Let’s Talk
            </a>
          </div>
        </div>
